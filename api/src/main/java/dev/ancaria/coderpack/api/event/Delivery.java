 ...
 * compiler; these two marks are about the hook behind the event instead -- how
 * completely it fires, and how often -- which is a fact about the agent and
 * about this loader version, not about the contract.
 *
...
public final class Delivery {
     ...
     * completeness. So the mistake to avoid is reading absence as absence --
     * "no {@link Stored} arrived, therefore nothing entered the bag" is exactly
     * the inference this mark withdraws. A mod that must not miss one should
     * confirm the state another way, or accept a gap.
     *
     * <p>The mark comes off as coverage lands. Its absence means the paths are
     * covered as far as the hooks know, not that a hook can never miss.
     */
    @Documented
     ...
     * Every listener on it runs on the loader's one dispatch thread, so whatever
     * a body allocates or waits for here it pays for continuously, and on the
