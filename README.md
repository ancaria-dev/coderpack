...
изменить или отменить. Файлы игры остаются нетронутыми, а после закрытия
...
public final class DoubleGold implements SacredMod {
    ...
    public void onGold(Gold event) {
        if (!event.spending()) {
            event.delta(event.delta() * 2);   // переписано до того, как игра запишет
        }
    }
}
```

Поддерживаемый метод с `@Subscribe` публичен, ничего не возвращает и принимает
ровно один объект события. Тип параметра задаёт подписку, поэтому строковое имя
события указывать не приходится. Для динамической регистрации есть
`context.events().on(...)`. Он возвращает `Handle`, через который слушатель
можно снять с регистрации.

Слушатели выполняются по приоритетам `FIRST`, `NORMAL`, `LAST`, `MONITOR`.
Внутри одного приоритета сохраняется порядок регистрации. Отмена события сама
по себе не прекращает рассылку. Параметр `ignoreCancelled = true` пропускает
слушатель, если событие уже отменено. На этапе `MONITOR` видны все предыдущие
решения, но новые отмены и изменения отбрасываются. Попытка изменить событие
попадает в журнал один раз для каждого слушателя.

Изменять или отменять можно `Gold`, `Experience`, `Damage`, `Skill`,
`Attribute` и `Pickup`. События `LevelUp`, `Hero`, `World`, `Position`,
`Moved`, `Death`, `NearDeath`, `MobHit`, `MobDeath`, `Equip` и `Stored`
доступны только для чтения. Неизвестное загрузчику строковое событие не
теряется. Оно приходит подписчикам `Event` как `Unknown`.

За команды в игру отвечает `context.game()`. Метод `player()` возвращает
кешированное состояние героя и операции `teleport`, `gold`, `hp`, `addExp`.
Методы `uiString`, `typeName`, `typeId` и `types` читают таблицы игры, а
`retype` меняет название типа и внешний вид существующего предмета. Изменение
сохраняется, но исходное поведение и модификаторы предмета остаются прежними.
Команды ждут ответ до двух секунд.

Во время обработки событий с правом вето игровой поток остановлен. Хост ждёт
...
хост выводит строку как есть. Туда же попадает и всё остальное, что мод скажет
в консоль: загрузчик перенаправляет `System.out` в stderr до того, как загрузит
первый мод. Кадры протокола идут по stdout, и чужая строка способна разрезать
кадр пополам.

Настраивать под это логгер не нужно. Log4j2 и Logback по умолчанию направляют
`ConsoleAppender` в `System.out`, но читают его при своей первой настройке,
которая происходит уже после перенаправления. slf4j-simple и
`java.util.logging` и так пишут в stderr. Разрушить поток по-прежнему способна
только запись в дескриптор 1 напрямую, мимо `System.out`.
...
`addr.py` создаёт таблицу из 26 RVA и трёх глобальных адресов. Он также
переносит в `agent/src/gen/addr.js` отпечаток поддерживаемой сборки игры.
Сейчас он описывает `pureHD.exe` версии 2.0.2.118 и содержит 20
