...
Решать можно `Gold`, `Experience`, `Damage`, `Skill`, `Attribute` и `Pickup`.
События `LevelUp`, `Hero`, `World`, `Position`,
...
теряется. Оно приходит подписчикам `Event` как `Unknown`. Полный контракт
описан в [docs/EVENTS.md](docs/EVENTS.md).
